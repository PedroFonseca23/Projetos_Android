...
export const initDatabase = async () => {
  console.log(`📂 Iniciando Banco V7 em: ${FileSystem.documentDirectory}SQLite/${DB_NAME}`);
  ...
  try {
    ...
    const userCount = await db.getFirstAsync('SELECT COUNT(*) as c FROM users');
    console.log(`📊 Estado atual: ${userCount?.c || 0} usuários cadastrados.`);
...
export const addProduct = async (title, price, width, height, imageUri, userId) => {
  try {
    const id = Crypto.randomUUID();
    const w = String(width || '0');
    const h = String(height || '0');
    
  
    await db.runAsync(
      'INSERT INTO products (id, title, price, width, height, imageUri, userId) VALUES (?, ?, ?, ?, ?, ?, ?)', 
      [id, title, price, w, h, imageUri, userId]
    );
    
    console.log(`✅ Produto Salvo: ${title} (ID: ${id})`);
    return true;
  } catch (e) {
...
export const getDashboardStats = async () => {
  try {
    ...
    return { totalViews: 0, totalProducts: 0, totalUsers: 0, popularProducts: [] };
  }
};
...
export const getCartItems = async (userId) => {
  return await db.getAllAsync(`
    SELECT c.id as cartId, c.quantity, p.* FROM cart_items c
