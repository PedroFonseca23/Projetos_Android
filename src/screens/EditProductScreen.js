...
const EditProductScreen = ({ navigation, route }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateProduct(product.id, title, price, parseFloat(width), parseFloat(height), imageUri);
      navigation.goBack();
    } catch (e) {
      console.error(e);
      Alert.alert('Erro', 'Não foi possível atualizar o quadro.');
    }
  };
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.scrollContent}>
      ...
      <View style={s.form}>
        ...
        <SolidButton text="Salvar Alterações" onPress={handleSave} />
      </View>
    </ScrollView>
  );
};
